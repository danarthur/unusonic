...
- **Provider — Deepgram Nova-3** (switched from OpenAI Whisper on 2026-04-16 — see §20 decision 13). `POST /api/aion/capture/transcribe` accepts audio via multipart form, calls Deepgram's pre-recorded transcribe endpoint using `DEEPGRAM_API_KEY`, returns `{ transcript, duration_seconds, keyterms_count }`. Server-only.
- **Keyterm prompting (the whole reason we picked Deepgram).** On each request, the route fetches the workspace's most recently touched `directory.entities.display_name` values, deduped and capped at 100, and passes them as Nova-3's `keyterm` parameter. This is the single highest-leverage configuration available for our failure mode — proper-noun accuracy on workspace-specific client names. Vendor-claimed 6× lift / up to 90% keyword recall rate vs. baseline. The feature is Nova-3-exclusive; older Deepgram models have a weaker `keywords` boost that we don't use.
- **Cost envelope.** Nova-3 batch is $0.0043/min ($0.258/hr) — ~30% cheaper than Whisper, irrelevant at expected volume (pennies per workspace per month). Latency is ~2–5× faster than Whisper end-to-end for short clips. Both are nice bonuses; the reason for the switch is accuracy on workspace vocabulary.
...
- **Fallback model.** If Nova-3 errors or the keyterm query fails, the route still ships a transcript (keyterms default to empty — Nova-3 runs with baseline vocabulary). If Nova-3 itself is unavailable, the feature fails closed (502) — we don't silently downgrade to a worse model without user awareness.
...
13. **Deepgram Nova-3 with Keyterm Prompting, not OpenAI Whisper.** Research pass 2026-04-16 (single-agent Field Expert dive into the 2026 STT landscape) converged on the same failure mode Daniel's dogfood surfaced: Whisper baseline can't reliably get "Henderson" vs. "Anderson" when both are plausible, and OpenAI's `prompt` param (224-token cap) is not a real per-request vocabulary-bias mechanism — OpenAI's own cookbook warns against using it as a spelling guide. Deepgram Nova-3's Keyterm Prompting is purpose-built for this exact problem: up to 100 terms per request, vendor-claimed 6× accuracy lift / up to 90% keyword recall on domain vocabulary, 2–5× faster than Whisper, ~30% cheaper at batch. Our workspaces have ~80 entities on average — fits under the cap with margin. Alternative fallbacks considered and rejected: audio-in-LLM (Gemini / GPT-4o-audio) loses the debuggable transcript artifact and has no proper-noun mechanism; Rev AI human-in-the-loop breaks the sub-second latency requirement; AWS Transcribe has 15s minimum billing hostile to short clips. See §10.7.
