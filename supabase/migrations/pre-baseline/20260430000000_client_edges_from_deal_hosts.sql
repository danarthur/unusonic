-- =============================================================================
-- Backfill + fix: CLIENT edges from workspace org to host persons.
--
-- The v3 create_deal_complete RPC (20260421000300) writes ops.deal_stakeholders
-- rows and CO_HOST pair edges for romantic/co_host hosts, but never writes
-- CLIENT edges from the workspace org entity to the host persons. The Network
-- page filters cortex.relationships to {PARTNER, VENDOR, CLIENT, VENUE_PARTNER}
-- sourced from the workspace org, so host-person entities created via the new
-- flow were invisible on /network until a manual backfill.
--
-- Confirmed 2026-04-19 on deal 238cabce-78c4-4be4-999c-47436f37437c
-- ("Ally & Emily Wedding"): stakeholders + CO_HOST present, zero CLIENT edges.
--
-- This migration:
--   1. Recreates create_deal_complete (v3 signature) with a CLIENT-edge loop
--      appended after the CO_HOST block. Only person-type hosts get CLIENT
--      edges — company hosts are out of scope here (see CLAUDE.md notes,
--      tracked separately).
--   2. One-shot backfill: inserts missing CLIENT edges for every existing
--      person stakeholder in role host / day_of_poc / bill_to whose workspace
--      org is non-null. The prior scoped fix was manual; this covers the
--      population.
--
-- Notes on earlier RPC versions:
--   - v2 (20260420030000) had the same gap. Its signature was dropped by v3,
--     so no live call site hits it.
--   - v1 (20260411210000) DID write CLIENT edges but only for the single
--     p_client_entity it accepted. Its signature was dropped by v2.
--   Only v3 is resolvable via supabase.rpc('create_deal_complete', ...).
--
-- Idempotent: cortex.relationships has UNIQUE (source_entity_id,
-- target_entity_id, relationship_type). Uses ON CONFLICT DO NOTHING so
-- re-running on the manually-backfilled deal won't clobber or duplicate.
-- =============================================================================
...
BEGIN
  ...
  -- 12. CO_HOST edges (unchanged from v2/v3)
  ...
  --      Without these, wedding couples (and any person-host deal) don't
  --      surface on /network, which filters cortex.relationships by
  --      {PARTNER, VENDOR, CLIENT, VENUE_PARTNER} sourced from workspace org.
  --      Company hosts are intentionally out of scope: the legacy
  --      public.deals.organization_id denorm still carries them, and the
  --      broader deal→company CLIENT edge story is tracked separately.
  --      Workspace org entity = first non-ghost company entity owned by the
  --      workspace; matches the convention in create_deal_complete_rpc.sql
  --      (v1) and createGhostVenueEntity.
...
COMMENT ON FUNCTION public.create_deal_complete(uuid, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, jsonb, text, text, jsonb) IS
  'v3.1: v3 + CLIENT edges from workspace org to each host person. Closes the gap where wedding couples created via the multi-date deal flow did not surface on /network. Company-host CLIENT edges intentionally out of scope.';

-- =============================================================================
-- One-shot backfill: CLIENT edges for person stakeholders on existing deals.
--
-- Scope: every ops.deal_stakeholders row with role IN (host, day_of_poc,
-- bill_to) whose entity is a person and whose workspace has a non-ghost
-- company entity (the "workspace org") but no CLIENT edge from that org to
-- the person. Wider than the RPC fix on purpose — historic deals may have
-- person bill_to / POC entities that are also stranded off the Network page.
--
-- Note: the RPC fix (above) only writes CLIENT edges for host persons. If we
-- later want POC / bill_to persons to auto-appear on Network too, extend the
-- RPC — the broader backfill here just cleans up what's already orphaned.
--
-- Idempotent via ON CONFLICT on the UNIQUE (source, target, type) index.
-- =============================================================================
...
SELECT
  ws_org.id AS source_entity_id,
  e.id      AS target_entity_id,
  'CLIENT'  AS relationship_type,
  jsonb_build_object(
    'tier', 'preferred',
    'introduced_via_deal_id', ds.deal_id
  ) AS context_data
...
JOIN LATERAL (
  SELECT id
  FROM directory.entities
  WHERE owner_workspace_id = d.workspace_id
    AND type = 'company'
    AND (attributes->>'is_ghost' IS NULL OR attributes->>'is_ghost' <> 'true')
  LIMIT 1
) ws_org ON true
WHERE ds.role IN ('host', 'day_of_poc', 'bill_to')
  AND e.type = 'person'
  AND ds.entity_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM cortex.relationships r
    WHERE r.source_entity_id = ws_org.id
      AND r.target_entity_id = e.id
