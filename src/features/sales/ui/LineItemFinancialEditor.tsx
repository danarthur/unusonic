...
/**
 * LineItemFinancialEditor — Glass slide-over panel for inspecting and editing a single
 * proposal line item. Triggered by clicking any row in the receipt.
 *
 * Design contract:
 * - Fixed right-side overlay (z-50), full viewport height
 * - Entrance: slides in from right with UNUSONIC_PHYSICS spring
...
export function LineItemFinancialEditor({
  ...
  // Sync local draft state when selected item changes
  useEffect(() => {
    if (item == null) return;
    ...
    setSwapOpen(false);
    setSwapError(null);
  }, [selectedIndex]); // keyed on index, not item reference

  // Close on Escape
  const panelRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && isOpen) onSelectIndex(null);
    };
    document.addEventListener('keydown', handleKey);
    return () => document.removeEventListener('keydown', handleKey);
  }, [isOpen, onSelectIndex]);
      ...
      {isOpen && item && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-obsidian/30 backdrop-blur-[2px]"
          ...
          <motion.div
            ...
            className="fixed inset-y-0 right-0 z-50 w-full max-w-[400px] flex flex-col border-l border-white/10 shadow-2xl"
            style={{
          ...
          >
            ...
            <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-4 border-b border-white/10">
              <div className="flex items-center gap-2 min-w-0">
                <Calculator className="w-4 h-4 text-ink-muted shrink-0" aria-hidden />
                <h2 className="text-xs font-semibold uppercase tracking-widest text-ink-muted truncate">
                  Line item
                </h2>
                {category && (
                  <span className="ml-1 shrink-0 rounded-full border border-white/10 bg-white/[0.06] px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-ink-muted">
                    {category.replace('_', ' ')}
                  </span>
                )}
              </div>
              <button
                type="button"
                onClick={() => onSelectIndex(null)}
                className="shrink-0 p-1.5 rounded-lg text-ink-muted hover:text-ink hover:bg-white/[0.08] transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
              ...
              <AnimatePresence mode="wait" initial={false}>
                ...
                >
                  ...
                  <div>
                    <label
                      htmlFor="editor-name"
                      className="block text-xs font-medium uppercase tracking-wider text-ink-muted mb-1.5"
                    >
                      Item name
                    </label>
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian read-only:opacity-70"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian resize-none leading-relaxed read-only:opacity-70"
                    />
                  </div>
                  ...
                  {!readOnly && (
                    <div>
                      <label
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-ink-muted mb-1.5"
                      >
                        <Lock className="w-3 h-3 opacity-60" aria-hidden />
                        Internal notes
                        <span className="ml-1 text-[10px] normal-case tracking-normal text-ink-muted/60">
                          (not on client PDF)
                        </span>
                      </label>
                      <textarea
                        ...
                        className="w-full rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-ink placeholder:text-ink-muted focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian resize-none leading-relaxed"
                      />
                    </div>
                  )}
                    ...
                    <div className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
                      <div className="flex items-center gap-2 min-w-0">
                        {item.isClientVisible !== false ? (
                          <Eye className="w-4 h-4 text-ink-muted shrink-0" aria-hidden />
                        ) : (
                          <EyeOff className="w-4 h-4 text-ink-muted/50 shrink-0" aria-hidden />
                        )}
                      ...
                      <button
                        ...
                        className={cn(
                          'shrink-0 relative inline-flex h-5 w-9 items-center rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian',
                          item.isClientVisible !== false
                            ? 'bg-[var(--color-neon)]'
                            : 'bg-white/20'
                        )}
                      >
                        <span
                          className={cn(
                            'inline-block h-3.5 w-3.5 rounded-full bg-obsidian shadow transition-transform',
                  ...
                  <div>
                    ...
                    <CurrencyInput
                      ...
                      placeholder="0.00"
                      disabled={readOnly}
                      className={cn(belowFloor && 'border-[var(--color-neon-amber)]/60')}
                    />
                    {belowFloor && floorPrice != null && (
                      <div className="mt-1.5 flex items-center gap-1.5 text-xs text-[var(--color-neon-amber)]">
                  ...
                  <div>
                    ...
                    <div className="flex items-start gap-2">
                      <select
                        ...
                        className="flex-1 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian disabled:opacity-60"
                      >
                      ...
                      {(item.unitType === 'hour' || item.unitType === 'day') && (
                        <div className="flex flex-col items-center gap-0.5 shrink-0">
                          <input
                            ...
                            className="w-20 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-sm text-ink text-center tabular-nums focus:outline-none focus:ring-2 focus:ring-[var(--ring)] focus:ring-offset-2 focus:ring-offset-obsidian [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none disabled:opacity-60"
                  ...
                  {(item.unitType === 'hour' || item.unitType === 'day' || item.quantity > 1) && (
                    <div className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3">
                      ...
                      {item.unitType === 'flat' ? (
                        <p className="text-xs text-ceramic tabular-nums">
                          {item.quantity} ×{' '}
                          <span className="text-ink">${effectivePrice.toLocaleString()}</span>
                          {' = '}
                          <span className="font-semibold text-ink">${lineTotal.toLocaleString()}</span>
                        </p>
                      ) : (
                        <p className="text-xs text-ceramic tabular-nums">
                          ...
                          <span className="text-ink">${effectivePrice.toLocaleString()}/{item.unitType === 'hour' ? 'hr' : 'day'}</span>
                          {' = '}
                          <span className="font-semibold text-ink">${lineTotal.toLocaleString()}</span>
                        </p>
                      )}
                      {hasCostData && totalCost != null && (
                        <p className="mt-1 text-xs text-ink-muted tabular-nums">
                          ...
                          = <span className="text-ceramic">${totalCost.toLocaleString()} cost</span>
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {!costHidden && (
                    <div>
                      ...
                      >
                        Cost per unit{' '}
                        <span className="normal-case tracking-normal font-normal text-ink-muted/60">
                          (what you pay)
                        </span>
                      </label>
                      <CurrencyInput
                      ...
                      {costRentalRetail && !readOnly && (
                        <label className="mt-2 flex items-center gap-2 cursor-pointer text-xs text-ink-muted select-none">
                          <input
                            type="checkbox"
                  ...
                  {costHidden && (
                    <div className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3 text-xs text-ink-muted">
                      {category === 'package'
                  ...
                  {!costHidden && hasCostData && totalCost != null && lineTotal > 0 && (
                    <div className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3 space-y-2">
                      ...
                      {/* Margin row */}
                      <div className="border-t border-white/10 pt-2 flex items-center justify-between gap-2 text-xs font-medium text-ceramic">
                  ...
                  {!costHidden && !hasCostData && (
                    <p className="text-xs text-ink-muted rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3">
                  ...
                  {showCrew && (
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        <Users className="w-3.5 h-3.5 text-ink-muted" aria-hidden />
                      ...
                      <div className="space-y-2">
                        {roles.map((role, i) => {
                          ...
                          return (
                            ...
                            >
                              <div className="min-w-0">
                                <p className="text-xs font-medium text-ceramic truncate">
                                ...
                                {role.assignee_name && (
                                  <p className="text-[11px] text-[var(--color-neon-amber)] mt-0.5 truncate">
                                    → {role.assignee_name}
                                  </p>
                                )}
                              </div>
                  ...
                  {isPackageHeaderOrChild && !readOnly && (
                    <p className="text-[11px] text-ink-muted/60 pt-1 border-t border-white/10">
            ...
            {(prevIndex != null || nextIndex != null) && (
              <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-3 border-t border-white/10">
                <button
                  type="button"
                  disabled={prevIndex == null}
                  onClick={() => prevIndex != null && onSelectIndex(prevIndex)}
                  className="inline-flex items-center gap-1.5 text-xs text-ink-muted hover:text-ceramic disabled:opacity-30 disabled:pointer-events-none transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ring)] rounded-lg px-2 py-1"
                >
                  <ChevronLeft className="w-3.5 h-3.5" aria-hidden />
                  Prev
                </button>
                <span className="text-[11px] text-ink-muted/60 tabular-nums">
