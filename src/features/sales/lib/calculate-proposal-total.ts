...
/**
 * Total expected cost of a proposal: sum of `actual_cost × quantity` for every
 * line item that isn't a bundle header. `actual_cost` is seeded from the
 * catalog's `target_cost` at add time by `addPackageToProposal`, so this is
