...
import type { Cue, CueType, RosTemplate } from '../model/run-of-show-types';
...
export async function saveRosTemplate(
  ...
  const templateCues = cues.map(({ title, duration_minutes, type, notes, is_pre_show, assigned_crew, assigned_gear, start_time }) => ({
    title,
    duration_minutes,
    type,
    notes,
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      name,
      description,
      cues: templateCues as unknown as Json,
    })
    .select('*')
    .single();
