...
export async function recordAionAction(workspaceId: string): Promise<void> {
  ...
  // Call .rpc / .from directly on the client (don't extract into a local
  // variable — that detaches `this` and the supabase-js internals fail with
  // "Cannot read properties of undefined (reading 'rest')"). We cast the
  // results to the shapes we expect so call-sites stay ergonomic.
  ...
  if (error) {
    ...
    if (data) {
      ...
      await (
        system.from('workspaces') as unknown as {
          update: (values: Record<string, unknown>) => {
            eq: (col: string, val: string) => Promise<{ error: { message: string } | null }>;
          };
        }
      )
        .update({ aion_actions_used: current + 1 })
        .eq('id', workspaceId);
    }
  }
}
