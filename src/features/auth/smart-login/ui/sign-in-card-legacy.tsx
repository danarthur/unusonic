/**
 * Sign-In Card (legacy) — passkey-first auth with password fallback.
 *
 * Preserved verbatim during the Phase 4 cutover. Renders when
 * `AUTH_V2_LOGIN_CARD` is OFF (the rollback path). The new
 * state-machine card lives in `sign-in-card.tsx` and delegates to this
 * module when the flag is disabled.
 *
 * Do not extend this file — any new behavior goes in the v2 card.
 * Deletion happens after the flag ships to 100% and stays stable.
 *
 * @module features/auth/smart-login/ui/sign-in-card-legacy
 */
...
export function LegacySignInCard({
  email,
  setEmail,
  redirectTo,
            ...
            <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="stage-input pr-11 pointer-events-auto cursor-text"
                ...
                >
                  {!otpSent ? (
                    <div className="space-y-4">
                      <p className="text-sm text-[var(--stage-text-secondary)] text-center">
                        6-digit code to <span className="text-[var(--stage-text-primary)]">{email}</span>
                      </p>
                      {otpError && (
                        <div className="stage-panel-nested stage-stripe-error p-3">
                      ...
                      >
                        {isOtpPending ? (
                          <><Loader2 className="w-4 h-4 animate-spin" strokeWidth={1.5} /> Sending code…</>
                        ) : (
                          'Send code'
                        )}
                      </motion.button>
                    ...
                    <div className="space-y-4">
                      <p className="text-sm text-[var(--stage-text-secondary)] text-center">
                        Code sent to <span className="text-[var(--stage-text-primary)]">{email}</span>
                      </p>
                      <p className="text-xs text-[var(--stage-text-secondary)] text-center">
                      ...
                      >
                        {isOtpPending ? (
                          ...
                          <>Verify code <ArrowRight className="w-4 h-4" strokeWidth={1.5} /></>
                        )}
                      </motion.button>
                      <button
                        type="button"
                        ...
                        disabled={isPending}
                        className="w-full text-sm text-[var(--stage-text-secondary)] hover:text-[var(--stage-text-primary)] transition-colors disabled:opacity-45"
                      >
                        Resend code
                      </button>
                    </div>
              ...
              <p className="text-center">
                <button
                  ...
                  disabled={isPending}
                  className="text-sm text-[var(--stage-text-secondary)] hover:text-[var(--stage-text-primary)] transition-colors disabled:opacity-45 disabled:cursor-not-allowed"
                >
                  Create account
                </button>
              </p>
