...
import { getDealClientContext } from '@/app/(dashboard)/(features)/events/actions/get-deal-client';
import { getDealCrew } from '@/app/(dashboard)/(features)/events/actions/deal-crew';
...
export function createDealKnowledgeTools(ctx: AionToolContext, helpers: ResolveHelpers) {
  ...
  const get_deal_signals = tool({
    ...
    execute: async (params) => {
      ...
      const { getDealSignals } = await import('@/app/(dashboard)/(features)/events/actions/get-deal-signals');
