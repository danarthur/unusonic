...
import { STAGE_HEAVY, M3_EASING_ENTER } from '@/shared/lib/motion-constants';
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-1">
        <h2 className="text-2xl font-medium tracking-tight text-[var(--stage-text-primary)]">Plan</h2>
      ...
      {persona && recommendedTier !== activeTier && (
        <motion.div
          ...
          className="flex items-start gap-3 rounded-2xl border border-[var(--stage-border-hover)] bg-[var(--stage-surface-nested)] px-4 py-3"
        >
          <Sparkles className="w-4 h-4 text-[var(--stage-accent)] mt-0.5 shrink-0" strokeWidth={1.5} />
          <p className="text-sm text-[var(--stage-text-primary)]/80">
            Aion recommends{' '}
            <span className="text-[var(--stage-text-primary)] font-medium">{SUBSCRIPTION_TIERS[recommendedTier].label}</span>
        ...
        {MAIN_TIERS.map((tierId, i) => {
          ...
          return (
            <motion.div
              ...
              className={`relative flex flex-col rounded-[var(--stage-radius-panel)] border p-6 ${
                isCurrent
                  ? 'border-[var(--stage-border-hover)] bg-[var(--stage-surface)]'
                  : 'border-[var(--stage-border)] bg-[var(--stage-surface-nested)] hover:bg-[var(--stage-surface)]'
              }`}
            >
              {/* Badges */}
              ...
              <div className="flex-1 space-y-4">
                <div>
                  <p className="text-base font-medium tracking-tight text-[var(--stage-text-primary)]">{data.label}</p>
                  <p className="text-2xl font-semibold tracking-tight text-[var(--stage-text-primary)] mt-1">{data.price}</p>
                </div>

                <ul className="space-y-2">
                  {data.highlights.map((h) => (
                    <li key={h} className="flex items-start gap-2 text-sm text-[var(--stage-text-secondary)]">
      ...
      >
        <div className="space-y-0.5">
          <h3 className="text-sm font-medium tracking-tight text-[var(--stage-text-primary)]">Studio URL</h3>
          <p className="text-xs text-[var(--stage-text-secondary)]">Your workspace&apos;s public handle on Unusonic.</p>
        </div>

        <div className="stage-panel rounded-2xl divide-y divide-[var(--stage-border)]">
          {/* Current handle */}
          <div className="flex items-center justify-between px-5 py-4">
            <div className="space-y-0.5">
              <p className="text-xs font-medium uppercase tracking-widest text-[var(--stage-text-secondary)]">Handle</p>
              <p className="text-sm font-mono text-[var(--stage-text-primary)]">
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <div className="space-y-0.5">
              <p className="text-xs font-medium uppercase tracking-widest text-[var(--stage-text-secondary)]">Custom domain</p>
              <p className="text-sm text-[var(--stage-text-primary)]/40">
                {activeTier === 'venue_os'
            ...
            {activeTier === 'venue_os' ? (
              <a
                ...
                className="shrink-0 ml-4 px-3 py-1.5 rounded-full text-xs font-medium border border-[var(--stage-border)] text-[var(--stage-text-primary)]/70 hover:text-[var(--stage-text-primary)] hover:border-[var(--stage-border-focus)] transition-colors"
              >
                Contact us
              </a>
            ) : (
              <div className="shrink-0 ml-4 flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[var(--stage-border)] text-xs text-[var(--stage-text-secondary)]">
      ...
      <motion.div
        ...
        className="stage-panel-nested p-6 flex flex-col md:flex-row md:items-center gap-6"
      >
        <div className="flex-1 space-y-1">
          <div className="flex items-center gap-2">
            <p className="text-sm font-medium text-[var(--stage-text-primary)] tracking-tight">Autonomous</p>
            <span className="text-[10px] font-medium uppercase tracking-widest text-[var(--stage-text-secondary)] border border-[var(--stage-border)] px-2 py-0.5 rounded-full">
              Add-on
            </span>
          </div>
          <p className="text-sm text-[var(--stage-text-secondary)]">{SUBSCRIPTION_TIERS.autonomous.price}</p>
          <p className="text-xs text-[var(--stage-text-secondary)]/60 mt-2">
            Digital Workers, UnusonicPay, and Explainable AI — available on any plan. Contact us to enable.
          </p>
        </div>
        <a
          href="mailto:hello@unusonic.com?subject=Autonomous add-on"
          className="shrink-0 px-5 py-2.5 rounded-full text-sm font-medium border border-[var(--stage-border)] text-[var(--stage-text-primary)]/70 hover:text-[var(--stage-text-primary)] hover:border-[var(--stage-border-focus)] transition-colors text-center"
        >
          Contact us
        </a>
      </motion.div>
