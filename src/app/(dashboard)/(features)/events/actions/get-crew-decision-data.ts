    ...
    if (eventDate) {
      ...
      if (eventBookings) {
        ...
        for (const b of eventBookings) {
          // `entity_id` is nullable: a booking with nobody on it is nobody's
          // conflict, and was previously keyed into the map against null.
          if (b.entity_id && !availabilityMap.has(b.entity_id)) {
