/**
 * Legacy drag-studio variant of the proposal builder. Kept behind the
 * `crm.proposal_builder_drag` feature flag during Phase 1 so we can A/B
 * against the new palette-first studio (`proposal-builder-studio.tsx`)
 * and verify the design-doc kill criteria before Phase 2 rips out
 * `@hello-pangea/dnd` entirely.
 *
...
/**
 * Legacy Proposal Builder Studio — split view with catalog-on-left / receipt-on-right,
 * cross-pane drag via @hello-pangea/dnd. Rendered only when the workspace has
 * `crm.proposal_builder_drag = true`.
 *
 ...
 */
export function ProposalBuilderStudioLegacy({ deal, contacts = [], clientAttached: clientAttachedProp }: ProposalBuilderStudioProps) {
  ...
  // Kill-criteria telemetry: mark the session the first time this variant
  // mounts for this deal. Downstream events (first_add, add_success) are
  // emitted on successful drops below.
  useEffect(() => {
  ...
  const onDragEnd = useCallback(
    (result: DropResult) => {
      ...
      addPackageToProposal(deal.id, packageId)
        .then((r) => {
          if (r.success) {
            events.emitAddSuccess('drag', { package_id: packageId });
    ...
    [deal.id, isLocked, packages, refetchProposal, events]
  );
