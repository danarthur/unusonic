  ...
  return (
    <StagePanel elevated className="p-5 rounded-[var(--stage-radius-panel)] border border-[oklch(1_0_0_/_0.10)]">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-xs font-medium uppercase tracking-widest text-[var(--stage-text-secondary)]">Budget</h3>
      </div>
      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
        {budgetEstimated != null && (
          <div>
            <p className="stage-label text-[var(--stage-text-tertiary)] truncate">Estimated</p>
            <p className="text-sm font-medium tracking-tight text-[var(--stage-text-primary)] tabular-nums">{fmt(budgetEstimated)}</p>
          </div>
        )}
        {proposalTotal != null && (
          <div>
            <p className="stage-label text-[var(--stage-text-tertiary)] truncate">Quoted</p>
            <p className="text-sm font-medium tracking-tight text-[var(--stage-text-primary)] tabular-nums">
              {fmt(proposalTotal)}
              {delta != null && (
        ...
        {ledgerActual != null && (
          <div className="col-span-2">
            ...
            <p className="text-sm font-medium tracking-tight text-[var(--stage-text-primary)] tabular-nums">{fmt(ledgerActual)}</p>
          </div>
        )}
      </div>
    </StagePanel>
